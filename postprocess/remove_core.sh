...
cd ..
python sat_dataprocess.py --cnf $cnf_file --core $core_file --save $save_file --origin $origin_core --add_var
rm $drat_file
rm $core_file
...
while [ "$no" -lt $num_iter ]; do
  ...
  ./drat-trim ../$cnf_file ../$drat_file -c ../$core_file
  cd ..
  python sat_dataprocess.py --cnf $cnf_file --core $core_file --save $save_file --origin $origin_core
