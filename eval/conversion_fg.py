...
def main():
    ...
    if action == "sat2fg":
        ...
        for i, graph in enumerate(graphs):
            ...
            FG_to_sat(graph, "{}/{}.cnf".format(store_dir, graph_names[i]), par_split)
    else:
...
def FG_to_sat(graph, save_name, par_split):
    ...
    for node in nodes:
        if (node >= num_var):
            ...
            assert(len(neighbors) > 0)
            for lit in neighbors:
                if graph.edges[lit, node]['features'] == [1,0,0]:
                    clause += "{} ".format(lit + 1)
                elif graph.edges[lit, node]['features'] == [0,1,0]:
                    clause += "{} ".format(-(lit + 1))
            clause += "0\n"
            clauses.append(clause)
...
def preprocess_FG(formula, FG, num_vars):
    ...
    for cn in range(len(formula)):
        for var in formula[cn]:
            if var > 0:
                FG.add_edge(var, cn+num_vars+1, features=[1,0,0])
            elif var < 0:
                FG.add_edge(abs(var), cn+num_vars+1, features=[0,1,0])
