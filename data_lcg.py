            ...
            while True:
                ...
                if np.random.rand()<self.yield_prob:
                    ...
                    if self.speedup and not self.core_flag:
                        ...
                        # x2 = torch.zeros((graph.number_of_nodes(), 11))
                        # for i in range(graph.number_of_nodes()):
                        #     if i < len(nodes_par1):
                        #         x2[i, c_labels[i]] = 1
                        #     else:
            ...
            while True:
                ...
                if np.random.rand()<self.yield_prob:
                    ...
                    if self.speedup and not self.core_flag:
                        ...
                        # for cmt in nodes_community:
                        #     if cmt == node_par1_cmt:
                        #         continue
                        #     else:
                        #         nodes_candidates = nodes_candidates - set(get_neigbors(graph, cmt, depth=2)[2])
                        node_sample = random.sample(nodes_candidates, k=1)[0]
                        if self.core_flag:
                            while node_sample in core_indexes['clause']:
                                node_sample = random.sample(nodes_candidates, k=1)[0]
...
class graph_generator():
    ...
    def resample(self, clause_num, flag_intercmt=True):
        ...
        while True:
            ...
            # sample additional node
            # if len(self.node_par2s) % 10 == 0:
            #     print(len(nodes_same_cmt))
            #     print(len(nodes_candidates))
            #     print(len(nodes_candidates - nodes_same_cmt))
            #     print(len(nodes_candidates.intersection(nodes_same_cmt)))
            #     print([len(set(get_neigbors(self.graph, x, depth=2)[2])) for x in self.nodes_community])
            #     print("\n")
            if flag_intercmt:
                ...
                for node_par1 in list(node_par1s):
                    ...
                    nodes_candidates = nodes_candidates - node_par1_nbrs - node_par1_not_nbrs
                    # nodes_candidates = nodes_candidates - nodes_same_cmt
            else:
                nodes_candidates = set(self.node_par2s) - {node_new}
                for node_par1 in list(node_par1s):
                    if node_par1 < len(self.nodes_par1) // 2:
    ...
    def update(self, node_pair, clause_num, flag_intercmt=True):
        ...
        if node_pair[0] in self.core_indexes['clause'] or node_pair[1] in self.core_indexes['clause']:
            # tmp1 = [(x+1 if x<len(nodes_par1s_train[idx])//2 else -1-(x - len(nodes_par1s_train[idx])//2)) 
    ...
    def get_graph(self):
        ...
        # print(edge_list)
        # edge_list = [pair for pair in edge_list if (pair[0] not in self.nodes_community and pair[1] not in self.nodes_community)]
