...
def save_graph_list(G_list, fname, file_names, clean=False, has_par=False, nodes_par1_list=None, nodes_par2_list=None):
    with open(fname, "wb") as f:
        ...
        for i,G in enumerate(G_list):
            ...
            if has_par:
                graphs_info.append([file_names[i], G.nodes(), G.edges(), nodes_par1_list[i], nodes_par2_list[i]])
            else:
                graphs_info.append([file_names[i], G.nodes(), G.edges()])
        pickle.dump(graphs_info, f)

def save_graph_list_fg(G_list, fname, file_names, clean=False, has_par=False, nodes_par1_list=None, nodes_par2_list=None):
    with open(fname, "wb") as f:
        graphs_info = []
        for i,G in enumerate(G_list):
            if clean:
                G = max(nx.connected_component_subgraphs(G), key=len)
            if has_par:
                graphs_info.append([file_names[i], G.nodes(), G.edges(data=True), nodes_par1_list[i], nodes_par2_list[i]])
            else:
                graphs_info.append([file_names[i], G.nodes(), G.edges(data=True)])
        pickle.dump(graphs_info, f)
...
def load_graph_list_fg(fname, has_par=False):
    with open(fname, "rb") as f:
        graphs = []
        if has_par:
            nodes_par1_list = []
